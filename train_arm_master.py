...
# use this file to train the arm's own muscles

def train_arm_master(env, lr_actor=0.001  , lr_critic=0.001 , num_episodes=100):
    ...
    def value_function_arm(state):
        n_hidden1 = 64 
        n_hidden2 = 24
        n_outputs = 1
        
        with tf.variable_scope("value_network_arm"):
    ...
    def policy_network_arm(state):
        n_hidden1 = 40
        n_hidden2 = 40
        # n_hidden3 = 40
        # n_hidden4 = 40
        # n_hidden5 = 10
        n_outputs = 6 # 2 actuators
    ...
    action_tf_var_arm, norm_dist = policy_network_arm(state_placeholder)
    V = value_function_arm(state_placeholder)
        ...
        for episode in range(num_episodes):
            ...
            while (not done) and steps<max_step:
                ...
                if np.random.random() > epsilon:
                    action  = sess.run(action_tf_var_arm, feed_dict={
            ...
            print("TRAIN ARM ONLY: Episode: {}, Number of Steps : {}, Average Cumulative reward: {:0.2f}".format(
    ...
    # plt.show()
    print("############ ARM ONLY TRAINING IS DONE! #######")
    print("total reward is", np.sum(np.array(episode_history)))

    return action_tf_var_arm, state_placeholder, episode_history
